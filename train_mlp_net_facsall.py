...
def test_model(epoch, net, num_classes1, dataset, data_loader, regression, batch_size, validation=False, fix_dist=None):
    ...
    for (img_features, labels, _, event_mask, binary_labels1, binary_labels2, annot_target) in data_loader:
        ...
        if regression:           
            ...
            loss1 = log_PDF_loss_weight*torch.mean(-dist.log_prob(labels.float()))
...
eval_dataset = FaceFeatureDataset(subject_under_test=subject_under_test, target_idx=target_idx , test=False,  data_dir='detected/', evaluating=False, use_holdout=True, threshold_percentile=threshold_percentile, balance_data=False, regression=regression, frames_pre=frames_pre, frames_after=frames_after, binary_classification=binary_classification)
    ...
    for i, (img_features, labels, _, event_mask, binary_labels1, binary_labels2, annot_targets) in enumerate(train_loader):   
        ...
        if regression:           
            ...
            if include_binary_loss:
                ...
                if include_aux_loss: loss = loss1 + 2*loss3
                else: loss = loss1
