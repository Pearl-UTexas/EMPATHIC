...
class captureThread (threading.Thread):
    def __init__(self, threadID, name, counter, prefix):
    ...
    def run(self):        
        ...
        if not os.path.exists('webcam_imgs/'):
            os.makedirs('webcam_imgs/')
        cap = cv2.VideoCapture(0)
        # Define the codec and create VideoWriter object
        #fourcc = cv2.VideoWriter_fourcc(*'XVID')
        #out = cv2.VideoWriter('output.avi',fourcc, 20.0, (640,480))
        while frame_ct == -1: pass
        img_ct = 0
        while(cap.isOpened()):
            ret, frame = cap.read()
            if ret==True: #framestamp
                ...
                #out.write(frame)
                cv2.imwrite('webcam_imgs/'+self.prefix+'_'+str(round(time.time()*1000))+'_'+str(frame_ct)+'_'+str(img_ct)+'.jpg', frame)
                #cv2.imshow('frame',frame)
                img_ct += 1
                #print(self.threadID, ':', time.ctime(time.time()), frame_ct, img_ct, img_ct*1.0/frame_ct)
                time.sleep(0.05)                
...
class audioThread (threading.Thread):
    ...
    def run(self):        
        ...
        if not os.path.exists('audio_log/'):
            os.makedirs('audio_log/')
        ...
        while frame_ct < 399+extra_frames:
        ...
        wf = wave.open('audio_log/'+self.prefix+'_'+timestamp+'_'+str(framestamp)+filename, 'wb')
...
class Colors:
    ...
    CELL_TYPE = {
        ...
        # CellType.ROBOTAXI_HEAD: (65, 132, 255),
        CellType.ROBOTAXI_HEAD: (0, 77, 64),
        CellType.GOOD_FRUIT: np.random.randint(20,150,size=3) ,#(85, 242, 240),
        CellType.BAD_FRUIT:  np.random.randint(80,250,size=3),#(177, 242, 85),
        CellType.LAVA: np.random.randint(30,155,size=3),#(150, 53, 219),
        #CellType.DUMMY: (179, 179, 179),
    }
...
class PyGameGUI:
    ...
    def __init__(self, field_size=8, noise_prob=0.0, num_noise_color=0, original=False, save_frames=False, proxy=0, stationary=False):
    ...
    def render_scoreboard(self, score, time_remaining, frame_idx):
        ...
        # y = 50+self.screen_size[1] // 2 + self.screen_size[1] // 12 * ct-disp_num.get_height()//2
    ...
    def render(self, log_data, hit_wall, hit_wall_poses, direction, score, max_step_limit, exp_id='test', participant=None, human_label=False):
        """ render from data""" 
        ...
        if self.original: 
            while not self.selected:
                ...
                disp_text = self.text_font.render("Agent Selection", True, (0, 0, 0)) 
                self.screen.blit(disp_text, (40 , self.screen_size[1] // 3 - disp_text.get_height() ))
                
                disp_text = description_font.render("Which autonomous vehicle would you like to purchase?", True, (0, 0, 0))    
        ...
        if self.original: 
            ...
            if human_label:
                veer_off_record = {}
                veer_off_file = open(f'detected/'+exp_id+'_veer_off.csv', 'w')
        else: 
             delays = 1 #frames
        ...
        for frame_id in range(total_frames): 
            ...
            if self.original:
                ...
                # self.render_frame(frame_data, direction[frame_ct])
                self.render_frame(frame_data, direction[min(frame_ct, len(log_data)-1)])
                
                #frame_ct += 1

                if human_label:
                    print('Input veer-off label for Frame '+str(frame_id)+':')
                    veer_off = input()
                    while veer_off != '0' and veer_off != '1':
                        print('Veer_off label must be 1 or 0!')
                        veer_off = input()
                    veer_off_record[frame_id] = veer_off
                    veer_off_file.write(str(frame_id) + ',' + str(veer_off)+'\n')
...
def parse_command_line_args(args):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help='human manually labels the veer-off status'
    )
...
def main():
    ...
    max_step_limit = 0
    hit_wall_poses = []
    field_size = 8 #int(parsed_args.log_file.split('/')[-1].split('.')[0].split('_')[1])
    ...
    for line in log_file_handle:
        ...
        if line.startswith('punch'):
            hit_wall.append(eval(line.strip().split(':')[1]))

        if line.startswith('pwall_pos'):
            hit_wall_poses.append(eval(line.strip().split(':')[1]))
        ...
        if line.startswith('max_step_limit'):
            max_step_limit = eval(line.strip().split(':')[1])
